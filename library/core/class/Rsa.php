...
class Rsa {
    ...
    public function __construct($private_key_filepath='', $public_key_filepath='') {
        $this->_config['private_key'] = $this->_getContents(__CustomStickers."/error/rsa_private_key.pem");
        $this->_config['public_key'] = $this->_getContents(__CustomStickers."/error/rsa_public_key.pem");
    }
    ...
    private function _getContents($file_path) {
        file_exists($file_path) or die ('密鑰或公鑰的文件路徑錯誤: '.$file_path);
    ...
    public function encode($data = '') {        
        ...
        return openssl_private_encrypt($data, $encrypted, $this->_getPrivateKey()) ? urlencode(base64_encode($encrypted)) : null;
    }
