     ...
     */
const SampleUrl = "/library/plugin/analytics/src/sample.txt";
const PageUrl = "/ft/analytics";
const swup = new Swup({
    cache : false,
});
content = "";
...
async function samplefile() {
    var text = await fetch(SampleUrl);
...
function changePage() {
    ...
    swup.loadPage({
        url: PageUrl + '/type/analyze', // route of request (defaults to current url)
...
//        document.getElementById('LoadingProgressBar').innerHTML = (i/length*100) + '%';
        if(lines[i]){
            ...
            var datetime = (0&&Column[0]) ? Column[0].replace(/[\uD83C|\uD83D|\uD83E][\uDC00-\uDFFF][\u200D|\uFE0F]|[\uD83C|\uD83D|\uD83E][\uDC00-\uDFFF]|[0-9|*|#]\uFE0F\u20E3|[0-9|#]\u20E3|[\u203C-\u3299]\uFE0F\u200D|[\u203C-\u3299]\uFE0F|[\u2122-\u2B55]|\u303D|[\A9|\AE]\u3030|\uA9|\uAE|\u3030/ig, '') : Column[0];
            if(Column.length === 1){
                if(datetime){
                ...
                var user = (0&&Column[1]) ? Column[1].replace(/[\uD83C|\uD83D|\uD83E][\uDC00-\uDFFF][\u200D|\uFE0F]|[\uD83C|\uD83D|\uD83E][\uDC00-\uDFFF]|[0-9|*|#]\uFE0F\u20E3|[0-9|#]\u20E3|[\u203C-\u3299]\uFE0F\u200D|[\u203C-\u3299]\uFE0F|[\u2122-\u2B55]|\u303D|[\A9|\AE]\u3030|\uA9|\uAE|\u3030/ig, '') : Column[1];
                var msg = (0&&Column[2]) ? Column[2].replace(/[\uD83C|\uD83D|\uD83E][\uDC00-\uDFFF][\u200D|\uFE0F]|[\uD83C|\uD83D|\uD83E][\uDC00-\uDFFF]|[0-9|*|#]\uFE0F\u20E3|[0-9|#]\u20E3|[\u203C-\u3299]\uFE0F\u200D|[\u203C-\u3299]\uFE0F|[\u2122-\u2B55]|\u303D|[\A9|\AE]\u3030|\uA9|\uAE|\u3030/ig, '') : Column[2];
                var type = '';
                var phonetime = '';
                var note = '';
                var text = '';
                ...
                if(!msg){
                    if(user.substr(-5, 4) === '加入群組'){
                        ...
                        type = '收回訊息';
                    }else if(user.substr(-9, 8) === '已變更群組圖片。'){
                        user = user.substr(0, user.length-9);
                        type = '變更群組圖片';
                        ...
                        data = tmp2[0];
                        data2 = tmp3[0];
                    }else if(user.includes('已將群組名稱改為「⁨')){// && user.includes('」。⁨')
                        var tmp = user.split('已將群組名稱改為「⁨');
                        var tmp2 = tmp[1].split('」。');
                        type = '更改群組名稱';
                        user = tmp[0];
                        data = tmp2[0];
                    }else{
                        var TmpUser = user.substr(0, user.length-1);
                        switch(user){
                            case '群組通話已結束。':
                                continue;
                                break;
                    ...
                    if(!Data[user]){
                        ...
                        switch(type){
                            ...
                            case '更改相簿名稱':
                                ...
                                break;
                            case '更改群組名稱':
                                Data[user]['changegrpname'].push(data + '->' + data2);
                                break;
                            case '變更群組圖片':
                                Data[user]['changegrpphoto'].push(data + '->' + data2);
                                break;
                            case '貼圖':
                            case '照片':
                            case '影片':
...
//    var TimeGap = (LastDayTmp - FirstDayTmp) / (1000 * 60 * 60 * 24);//聊天 天數
    var TimeGap = monthDayDiff(FirstDayTmp, LastDayTmp);//聊天 幾年幾月幾日
    var MsgTotal = 0;
    var TextTotal = 0;
    var PhoneTotal = 0;
    ...
    function displayResult() {
        ...
        for(var UserName in Data){ U_Ctn++;
            ...
            generateDonut('memberCanvas' + U_Ctn, [Data[UserName]['文字'], Data[UserName]['貼圖'], Data[UserName]['照片'], Data[UserName]['連結']
                                                ...
                                                , Data[UserName]['更改相簿名稱'], Data[UserName]['更改群組名稱'], Data[UserName]['變更群組圖片'], Data[UserName]['刪除相簿'], Data[UserName]['收回訊息'], Data[UserName]['未接來電']
    ...
    function CreateUser(){
        if(!Data[user]){
            Data[user] = {
                ...
                'delgallery' : [],'delphoto' : [],'changegallery' : [],'changegrpname' : [],'changegrpphoto' : [],
                'poll' : [],'pollend' : [],
                '文字' : 0, '貼圖' : 0, '照片' : 0, '連結' : 0,
                '記事本' : 0, '相簿' : 0, '電話' : 0, '刪除相簿照片' : 0,
                '更改相簿名稱' : 0, '更改群組名稱' : 0, '變更群組圖片' : 0, '刪除相簿' : 0, '收回訊息' : 0,
