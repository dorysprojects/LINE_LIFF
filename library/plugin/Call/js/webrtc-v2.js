...
const PHONE = window.PHONE = config => {
    ...
    const conversations = {};
    let   myvideo       = document.getElementById("localVideo");//document.createElement('video')
    let   snapper       = ()=>' ';
    //let   mystream      = null;//localStream
    ...
    function get_conversation(number) {
        let talk = conversations[number] || (number => {
            ...
            talk.connected = cb => {talk.connect = cb; return talk};//connected
    ...
    PHONE.dial = function( number, servers ) {
        ...
        if(localStream){
            localStream.getTracks().forEach(track => talk.pc.addTrack(track, localStream));
        }else{
            talk.pc.getSenders().forEach((sender) => talk.pc.removeTrack(sender));
        }
    ...
    PHONE.snapshots_setup = function (stream) {snapshots_setup(stream); };
    ...
    function onready(subscribed) {
        ...
        //if (!(localStream && myconnection)) return;

        connectcb();
        if (!autocam) readycb();
    }
    PHONE.onready = function (flag) {onready(flag); };
    ...
    if (autocam) startconnect();//startcamera
