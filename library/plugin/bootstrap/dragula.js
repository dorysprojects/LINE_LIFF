...
function dragula (initialContainers, options) {
  ...
  function drag (e) {
    ...
    var elementBehindCursor = getElementBehindPoint(_mirror, clientX, clientY);
    var dropTarget = findDropTarget(elementBehindCursor, clientX, clientY);
    if(!dropTarget && $(elementBehindCursor)[0] && $(elementBehindCursor).hasClass('panel-heading')){
        $(elementBehindCursor).parent().find('.Toggle').addClass('in').css('height', 'auto');
        $(elementBehindCursor).find('.ToggleBtn .btn .fa').removeClass('fa-angle-down').addClass('fa-angle-up');
        
        elementBehindCursor = dropTarget = $(elementBehindCursor).parent().find('.Toggle .DayTrip')[0];
    }
