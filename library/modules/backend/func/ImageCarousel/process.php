<?php
    ...
    function AddRow($_this, $_Module){
        ...
        for ($columnsList = 0; $columnsList < 10; $columnsList++) {
            if(($_this->my['fields']['subject']['img'.$columnsList]||$_FILES['img'.$columnsList]['tmp_name']) && $_this->my['fields']['subject']['subject'.$columnsList.'_0'] && $_this->my['fields']['subject']['action'.$columnsList.'_0'] && $_this->my['fields']['subject']['data'.$columnsList.'_0']){
                $TemplateCtn++;
            }
        }
        if($TemplateCtn===0){
            ...
            $errorMsg .= '至少一組大圖「需填寫:圖片、按鈕-標題、按鈕-動作、按鈕-內容」';
        }
        if($errorMsg){
            $return['state'] = false;
            $return['msg'] = $errorMsg;
