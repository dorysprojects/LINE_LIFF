...
<head>
    ...
    <link href="{#$__PLUGIN_Web#}/Call/css/bootstrap.min.css" rel="stylesheet" type="text/css">
    <link href="{#$__PLUGIN_Web#}/Call/css/styles.css" rel="stylesheet" type="text/css">
...
<div class="container">
    <div class="row">
        ...
        <div id="loginArea" class="col-xs-12">
            ...
            <div id="my-number-permalink" class="btn btn-warning loginItem" onclick="var ShareLink='{#$ShareLink#}'+btoa($('#my-number-link').attr('href'));window.open(ShareLink);">
                邀請好友通話
...
<!--<script src="{#$__PLUGIN_Web#}/Call/js/pubnub.js?{#$smarty.now|date_format:'%Y%m%d%H:%M:%S'#}"></script> -->
<script src="https://cdn.pubnub.com/pubnub.js"></script>
<!-- WebRTC SDK: https://github.com/stephenlb/webrtc-sdk -->
<script src="{#$__PLUGIN_Web#}/Call/js/webrtc-v2.js?{#$smarty.now|date_format:'%Y%m%d%H:%M:%S'#}"></script>
<script src="{#$__PLUGIN_Web#}/Call/js/sound.js"></script>
...
<script>(function(){
var ReplaceMent = (location.href.includes('/Call?')) ? '/Call?' : '/Call';
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
my_link.href  = location.href.split(ReplaceMent)[0] + '/Call?call=' + encodeURI(number) + NumParams + '&openExternalBrowser=1';
...
function urlparams() {
    ...
    PUBNUB.each(
        location.href.split(ReplaceMent)[1].split('&'),
...
function urlstring(params) {
    return location.href.split(ReplaceMent)[0] + '/Call?' + PUBNUB.map(
...
phone.debug(phone_debug);
    ...
    $(function () {
        ...
        $(document).on('click', '#SendMsgBtn', function(event) {
            if($('#SendMsgBtn').attr('connected')==='false'){
                dial(PUBNUB.$('number').value);
            }else{
        ...
        $(document).on('change', '#my-number, #number', function(event) {
            location.href = location.href.split(ReplaceMent)[0] + '/Call?call=' + encodeURI(PUBNUB.$('number').value) + '&number=' + encodeURI(PUBNUB.$('my-number').value) + '&openExternalBrowser=1';
        });
        $(document).on('click', '#remoteVideo, #localVideo', function(event) {
        ...
        $(document).on('click', '#volumeBtn', function(event) {
        ...
        $(document).on('click', '#videoBtn', function(event) {
            if($(this).attr("data-flag")==="on"){
                $(this).attr("data-flag", "off");
                $(this).removeClass('btn-danger').addClass('btn-primary');
        ...
        $(document).on('click', '#phoneBtn', function(event) {
        ...
        $(document).on('click', '#microphoneBtn', function(event) {
            if($(this).attr("data-flag")==="on"){
                $(this).attr("data-flag", "off");
                $(this).removeClass('btn-success').addClass('btn-danger');
        ...
        $(document).on('click', '#mirroredBtn', function(event) {
            if($(this).attr("data-flag")==="on"){
                $(this).attr("data-flag", "off");
                $(this).removeClass('btn-success').addClass('btn-danger');
            }else{
